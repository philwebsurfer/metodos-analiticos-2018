...
```{r}
log_prob(textos, n_gramas, n = 1, laplace = T)
log_prob(textos, n_gramas, n = 2, laplace = T)
```
...
_Unsmoothed: specially for bigrams, due to the fact that not always are the bigrams present in the corpus, even though they're valid language words. So, as we saw in class, those are not assigned 0, but instead a add a small $ \delta $.
...
sims <- big_model[1:5000,] %>%  # las 5000 palabras más comunes
        ...
        rownames_to_column() %>%
        tibble::as_data_frame()
similares <- sims %>% filter_at(vars(-rowname), any_vars(. > 0.21))  
ggplot(similares, aes(x = españa, y =madrid, label = rowname)) +
  geom_text_repel()
```
